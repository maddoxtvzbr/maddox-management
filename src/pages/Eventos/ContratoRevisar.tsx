...
import { validarSnapshotParaGeracao } from "../../lib/contratoValidacao";
...
export default function ContratoRevisar({
  ...
  async function handleVisualizar() {
    ...
    const mensagemFaltando = validarSnapshotParaGeracao(snapshot);
    if (mensagemFaltando) {
      setErro(mensagemFaltando);
      return;
    }
    ...
    try {
      ...
      console.error("[PDF][PREVIEW]", error instanceof Error ? error.message : error);
      setErro(
        error instanceof Error && error.message
          ? error.message
          : "Não foi possível gerar a prévia do contrato."
      );
    } finally {
      setCarregando(null);
    }
  ...
  async function handleGerar() {
    ...
    try {
      contrato = await gerarOuAtualizarContrato(evento.id, snapshot, autorizaImagem, foroFinal);
    } catch (error) {
      console.error("[PDF][SAVE_METADATA]", error instanceof Error ? error.message : error);
      setErro("Não foi possível salvar os dados do contrato. Verifique sua conexão e tente novamente.");
      setCarregando(null);
      return;
    }
    ...
    let blob: Blob;
    try {
      blob = await gerarContratoPdfBlob({
      ...
      console.error("[PDF][GENERATE_BLOB]", error instanceof Error ? error.message : error);
      setErro(
        error instanceof Error && error.message
          ? error.message
    ...
    // A prévia local já funcionou (Blob válido) — mostra ela já, independente
    // do upload ao Supabase ter sucesso ou não a seguir.
    abrirArquivoEmNovaAba(blob);

    try {
      const path = await enviarContratoOriginal(evento.id, contrato.id, blob);
      await salvarCaminhoOriginal(evento.id, path);
      console.log("[PDF][UPLOAD] OK", { path });
    } catch (error) {
      console.error("[PDF][UPLOAD]", error instanceof Error ? error.message : error);
      setErro(
        "O PDF foi gerado, mas não foi possível salvá-lo na sua conta. Verifique sua conexão e toque em \"Gerar PDF novamente\" mais tarde."
      );
      setCarregando(null);
      return;
    }
