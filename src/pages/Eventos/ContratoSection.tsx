...
import { validarDadosContrato, validarSnapshotParaGeracao } from "../../lib/contratoValidacao";
...
export default function ContratoSection({
  ...
  async function confirmarRegenerar() {
    ...
    const snapshotAtual = construirSnapshotContrato(evento, parcelas);
    const mensagemFaltando = validarSnapshotParaGeracao(snapshotAtual);
    if (mensagemFaltando) {
    ...
    let atualizado: Contrato;
    try {
      atualizado = await gerarOuAtualizarContrato(
        evento.id,
        snapshotAtual,
    ...
    let blob: Blob;
    try {
      blob = await gerarBlobDoContrato(atualizado);
    } catch (error) {
      console.error("[PDF][GENERATE_BLOB]", error instanceof Error ? error.message : error);
      setErro(
        error instanceof Error && error.message
          ? error.message
          : "Não foi possível gerar a nova versão do contrato."
      );
      setCarregando(null);
      setConfirmRegenerar(false);
      return;
    ...
    try {
      ...
      setErro(
        "O PDF foi gerado, mas não foi possível salvá-lo na sua conta. Verifique sua conexão e tente novamente."
      );
    } finally {
      setCarregando(null);
      setConfirmRegenerar(false);
