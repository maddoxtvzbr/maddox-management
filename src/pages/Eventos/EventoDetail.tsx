...
import { getEvento, listParcelasPorEvento, cancelarEvento } from "../../data/eventosRepository";
...
import { verificarPodeExcluirEvento, excluirEventoDefinitivamente } from "../../lib/exclusaoEvento";
import {
...
export default function EventoDetail({ eventoId, onBack, onIrParaConfiguracoes }: EventoDetailProps) {
  ...
  const [confirmCancelarEvento, setConfirmCancelarEvento] = useState(false);
  const [cancelandoEvento, setCancelandoEvento] = useState(false);
  const [confirmExcluirEvento1, setConfirmExcluirEvento1] = useState(false);
  const [confirmExcluirEvento2, setConfirmExcluirEvento2] = useState(false);
  const [excluindoEvento, setExcluindoEvento] = useState(false);
  const [erroGerenciamento, setErroGerenciamento] = useState<string | null>(null);
  ...
  async function handleConfirmarCancelarEvento() {
    if (cancelandoEvento) return;
    setCancelandoEvento(true);
    setErroGerenciamento(null);
    try {
      await cancelarEvento(eventoId);
      await reload();
    } catch (error) {
      console.error("[CANCELAR_EVENTO]", error);
      setErroGerenciamento("Não foi possível cancelar o evento. Verifique sua conexão e tente novamente.");
    } finally {
      setCancelandoEvento(false);
      setConfirmCancelarEvento(false);
    }
  }

  async function handleTocarExcluirEvento() {
    setErroGerenciamento(null);
    try {
      const verificacao = await verificarPodeExcluirEvento(eventoId);
      if (!verificacao.podeExcluir) {
        setErroGerenciamento(verificacao.motivo ?? "Este evento não pode ser excluído.");
        return;
      }
      setConfirmExcluirEvento1(true);
    } catch (error) {
      console.error("[EXCLUIR_EVENTO][VERIFICAR]", error);
      setErroGerenciamento("Não foi possível verificar o evento. Verifique sua conexão e tente novamente.");
    }
  }

  function handleConfirmarExcluirEvento1() {
    setConfirmExcluirEvento1(false);
    setConfirmExcluirEvento2(true);
  }

  async function handleConfirmarExcluirEvento2() {
    if (excluindoEvento) return;
    setExcluindoEvento(true);
    try {
      await excluirEventoDefinitivamente(eventoId);
      setConfirmExcluirEvento2(false);
      onBack();
    } catch (error) {
      console.error("[EXCLUIR_EVENTO]", error);
      setErroGerenciamento(
    ...
    <div className="orc-form-screen">
      ...
      <div className="orc-form-scroll">
        <section className="detail-hero card">
          <span className={`status-pill ${evento.status === "cancelado" ? "naofechou" : "fechado"}`}>
            {evento.status === "cancelado" ? "Cancelado" : "Confirmado"}
          </span>
        ...
        <section className="section-block">
          <p className="section-title">Gerenciar evento</p>
          {erroGerenciamento && <p className="field-error">{erroGerenciamento}</p>}
          <div className="gerenciar-evento-acoes">
            {evento.status !== "cancelado" && (
              <button
                type="button"
                className="btn-secondary tone-negative"
                onClick={() => setConfirmCancelarEvento(true)}
              >
                Cancelar evento
              </button>
            )}
            <button type="button" className="btn-secondary tone-negative" onClick={handleTocarExcluirEvento}>
              Excluir evento
            </button>
          </div>
        </section>
      ...
      <ConfirmSheet
        open={confirmCancelarEvento}
        title="Cancelar este evento?"
        description="O evento sai da Agenda e deixa de contar no Financeiro, mas o histórico e os dados já lançados são preservados."
        confirmLabel="Cancelar evento"
        tone="negative"
        confirming={cancelandoEvento}
        onCancel={() => setConfirmCancelarEvento(false)}
        onConfirm={handleConfirmarCancelarEvento}
      />

      <ConfirmSheet
        open={confirmExcluirEvento1}
        title="Excluir este evento permanentemente?"
        description="Esta ação não poderá ser desfeita."
        confirmLabel="Continuar"
        tone="negative"
        onCancel={() => setConfirmExcluirEvento1(false)}
        onConfirm={handleConfirmarExcluirEvento1}
      />

      <ConfirmSheet
        open={confirmExcluirEvento2}
        title="Confirme novamente: excluir de vez?"
        description="Não há como recuperar este evento depois de excluído. O orçamento de origem permanece no histórico."
        confirmLabel="Excluir definitivamente"
        tone="negative"
        confirming={excluindoEvento}
        onCancel={() => setConfirmExcluirEvento2(false)}
        onConfirm={handleConfirmarExcluirEvento2}
      />
